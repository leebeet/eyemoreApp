...
- (void)layoutSubviews {
    ...
    //自定义bar的粗细
    self.sliderLine.frame = CGRectMake(lineLeftSide.x, lineLeftSide.y, lineRightSide.x-lineLeftSide.x, 1.5);
...
- (void)updateLabelValues {
    ...
    //BLModified (int)强制转换selected值为整形，解决handle上方显示label数字不对的bug
    self.minLabel.string = [formatter stringFromNumber:@((int)self.selectedMinimum)];
    self.maxLabel.string = [formatter stringFromNumber:@((int)self.selectedMaximum)];
}
...
-(void)setTintColor:(UIColor *)tintColor{
    ...
    //自定义bar的颜色
    self.sliderLine.backgroundColor = [[UIColor grayColor] colorWithAlphaComponent:0.45].CGColor;
