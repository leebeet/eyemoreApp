...
@synthesize interaction, window, background, hud, spinner, image, label, view;
...
//BLModified
+ (void)show:(NSString *)status view:(UIView *)myview Interaction:(BOOL)Interaction
{
    [self shared].interaction = Interaction;
    [[self shared] hudMake:status view:myview spin:NO hide:NO];
}

- (void)hudMake:(NSString *)status view:(UIView *)myview spin:(BOOL)spin hide:(BOOL)hide
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    [self hudCreate];
    
    // clear view before showing
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    //BLModified clear view before showing
    [view removeFromSuperview]; view = nil;
	//---------------------------------------------------------------------------------------------------------------------------------------------
	label.text = status;
	label.hidden = (status == nil) ? YES : NO;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	if (label.text != nil)
	{
        ...
        //BLModified
        if (viewRect.size.width > labelRect.size.width) {
            hudWidth = viewRect.size.width + 24;
    ...
    //BLModified
    view.center = spinner.center;
	//---------------------------------------------------------------------------------------------------------------------------------------------
	label.frame = labelRect;
    
    //BLModified
    label.center = CGPointMake(spinner.center.x, label.center.y);
}
